...
function updateItemPrice(itemId, discountType) {
    ...
    // 平日割引とLINEクーポンの排他制御
    if (discountType === 'weekday' && weekdayCheckbox.checked) {
        ...
        // 他の施術の平日割引とLINEクーポンを解除
        Object.keys(selectedMenus).forEach(id => {
            if (id !== itemId) {
                if (selectedMenus[id].hasWeekdayDiscount) {
                    document.getElementById(`weekday_${id}`).checked = false;
                    selectedMenus[id].hasWeekdayDiscount = false;
                }
                // LINEクーポンも解除（平日割引との併用不可）
                ...
                if (otherLineCheckbox && otherLineCheckbox.checked) {
                    ...
                    selectedMenus[id].hasLineDiscount = false;
                    hasLineDiscount = false;
                }
            }
        });
    ...
    } else if (discountType === 'line' && lineCheckbox.checked) {
        ...
        // 他の施術のLINEクーポンと平日割引を解除
        Object.keys(selectedMenus).forEach(id => {
            if (id !== itemId) {
                if (selectedMenus[id].hasLineDiscount) {
                    const otherLineCheckbox = document.getElementById(`line_${id}`);
                    if (otherLineCheckbox) {
                        otherLineCheckbox.checked = false;
                        selectedMenus[id].hasLineDiscount = false;
                    }
                }
                // 平日割引も解除（LINEクーポンとの併用不可）
                const otherWeekdayCheckbox = document.getElementById(`weekday_${id}`);
                if (otherWeekdayCheckbox && otherWeekdayCheckbox.checked) {
                    otherWeekdayCheckbox.checked = false;
                    selectedMenus[id].hasWeekdayDiscount = false;
                    hasWeekdayDiscount = false;
                }
            }
